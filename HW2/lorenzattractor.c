...
float red = 1;      // Red value for visualizing lorenz equation.
float green = 0;    // Green value for visualizing lorenz equation.
float blue = 0;     // Blue value for visualizing lorenz equation.
...
void display()
{
   ...
   for (int i = 0; i < 50000; i++)
   {
      ...
      // Scale down the x, y, z coordinates to fit in our view.
      glVertex3d(x*0.02, y*0.02, z*0.02);
   }
   glEnd();
...
void key(unsigned char key, int x, int y)
{
   ...
   // Increase the specified lorenz parameter if '+' key is pressed.
   else if(key == '+')
   {

      // Increase the lorenz parameters based on the current mode
      // Used ternary operators to implement "wrap around behavior"
      // see README and write-up for more detail.
      switch(mode)
      {
         case 1:
            s = (s >= 200) ? -250 : s + 1;
   ...
   // Decrease the specified lorenz parameter if '-' key is pressed.
   else if(key == '-')
   {

      // Decrease the lorenz parameters based on the current mode
      // Used ternary operators to implement "wrap around behavior"
      // see README and write-up for more detail.
      switch(mode)
      {
   ...
   // If user presses 'c' key, switch between red, green and blue visualizations (for acessibility).
   else if (key == 'c')
   {
      // Toggle green visualization.
